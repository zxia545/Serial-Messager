...
class CommLonMsg():
    ...
    def _TEventSuper(self, sig, sendFromCtrl=False):
    ...
    def _send_and_validate_echo(self, packet):
        for i in range(self._max_echo_retries):
            ...
            if packet == self._commsRead(len(packet)):
                return True
            self._logger.warn(f'Retry for receiving hardware echo message - {i+1}')
    ...
    def _flush_serial_buffers(self):
        try:
            ...
            self._logger.error(f"Got a exception {e} when trying to reset input buffer")
    ...
    def setResMemBit(self, address: int, bit_position: int, reset: bool = False):
        ...
        for _ in range(self._maxNumOfTries):
            ...
            if rx_data == [0]:  # Assuming 0 means successful operation
